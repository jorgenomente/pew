...
interface AddCasitaItemModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (item: CasitaItemFormData) => void;
}
...
export function AddCasitaItemModal({ isOpen, onClose, onAdd }: AddCasitaItemModalProps) {
  ...
  const porcentajePagado = formData.montoEstimado && formData.pagado
    ? (parseFloat(formData.pagado) / parseFloat(formData.montoEstimado)) * 100
    : 0;
